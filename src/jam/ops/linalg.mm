...
void DotGeneral(Lowering& L, mlir::Operation* op) {
  ...
  plan(lshape, lb, lc, false, lp, lrs);
  plan(rshape, rb, rc, true, rp, rrs);
  (void)lr; (void)rr;
...
void Convolution(Lowering& L, mlir::Operation* op) {
  ...
  if (inSpatial.size() == 1) {
    ...
    s = [L.graph() reshapeTensor:s withShape:@[ @(N), @1, @(W), @(C) ] name:nil];
    if (transposed) {
    ...
    w = [L.graph() reshapeTensor:w withShape:@[ @1, @(KW), @(KI), @(KO) ] name:nil];
    ...
    o = [L.graph() reshapeTensor:o withShape:@[ @(oN), @(oW), @(oC) ] name:nil];
    std::vector<int64_t> nwcSlotOf(3, 0);
  ...
  std::vector<int64_t> wPerm = {kSpatial[0], kSpatial[1], dn.getKernelInputFeatureDimension(),
                                dn.getKernelOutputFeatureDimension()};
  MPSGraphTensor* w = Transposed(L, L.value(op->getOperand(1)), wPerm);
